...
test('the wardrobe is as big as the shop claims', () => {
  ...
  assert.equal(counts.hat, 23, 'hats: 22 plus bare');
  assert.equal(counts.outfit, 19, 'outfits: 18 plus bare');
  assert.equal(counts.accessory, 13, 'accessories: 12 plus bare');
  assert.equal(COSMETICS.length, 92);
});
